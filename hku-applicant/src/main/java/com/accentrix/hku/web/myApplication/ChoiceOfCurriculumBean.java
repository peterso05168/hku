...
public class ChoiceOfCurriculumBean implements Serializable {

    private static final long serialVersionUID = 1L;

    private static final Logger LOG = LoggerFactory.getLogger(ChoiceOfCurriculumBean.class);
    ...
    private void processDocumentRequirements() {
        boolean addUclDocReq = false;
        if (!isChoiceEndDate) {
            FormProgVo firstFormProgVo = formProgService.get(firstProgChoice.getAdmFormProgId());
            if (firstFormProgVo != null
                    && Constants.HKU_UCL_DUAL_DEGREE_PROG_CD.equals(firstFormProgVo.getProgrammeCode()))
                addUclDocReq = true;
            else if (CollectionUtils.isNotEmpty(othersProgChoices)) {
                for (ProgrammeChoiceVo otherProgChoice : othersProgChoices) {
                    if (otherProgChoice != null && StringUtils.isNotBlank(otherProgChoice.getAdmFormProgId())) {
                        FormProgVo otherFormProgVo = formProgService.get(otherProgChoice.getAdmFormProgId());
                        if (otherFormProgVo != null
                                && Constants.HKU_UCL_DUAL_DEGREE_PROG_CD.equals(otherFormProgVo.getProgrammeCode()))
                            addUclDocReq = true;
                    }
                }
            }
            if (addUclDocReq)
                addDocumentRequirement(Constants.DOC_TYPE_PIS, Constants.HKU_UCL_DUAL_DEGREE_PROG_CD,
                        "Personal Statment for HKU-UCL Dual Degree", applicationId);
            else
                deleteDocumentRequirement(Constants.DOC_TYPE_PIS, Constants.HKU_UCL_DUAL_DEGREE_PROG_CD,
                        "Personal Statment for HKU-UCL Dual Degree", applicationId);
        }

        if (sportScholarship != null && sportScholarship.getIsActiveSport())
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarSports", "Sports Scholar", applicationId);
        else
            deleteDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarSports", "Sports Scholar", applicationId);

        if (applicationVo.getScholarHeforshe() != null && applicationVo.getScholarHeforshe()) {
            if ("m".equals(personalParticularsVo.getSex()))
                addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "SupplementaryEssayMale",
                        ...
                        applicationId);
            else if ("f".equals(personalParticularsVo.getSex()))
            ...
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "AcademicTranscriptForScholarHeforShe",
        ...
        if (applicationVo.getScholarUwc() != null && applicationVo.getScholarUwc())
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarUWC", "UWC University Scholarship",
                    applicationId);
        else
            deleteDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarUWC", "UWC University Scholarship",
                    applicationId);
        if (applicationVo.getScholarNigerian() != null && applicationVo.getScholarNigerian())
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarNigerian", "Nigerian Scholarship", applicationId);
        else
        ...
        if (applicationVo.getScholarVtp() != null && applicationVo.getScholarVtp()) {
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "HRTR",
                    "VTP – Household Registry and Temporary Residence Record", applicationId);
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "HST", "VTP – High School Transcript of Year 10-12",
                    applicationId);
            ...
            deleteDocumentRequirement(Constants.DOC_TYPE_OTHERS, "HRTR",
                    "VTP – Household Registry and Temporary Residence Record", applicationId);
            deleteDocumentRequirement(Constants.DOC_TYPE_OTHERS, "HST", "VTP – High School Transcript of Year 10-12",
                    applicationId);
            if (!Constants.NOT_POST_SECONDARY_STUDIES_QUALIFICATION.contains(acadBgVo.getHighestQualificationCd()))
        ...
        if (applicationVo.getScholarAfl() != null && applicationVo.getScholarAfl())
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarAFL", "Asian Future Leaders Scholarship",
                    applicationId);
        else
            deleteDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarAFL", "Asian Future Leaders Scholarship",
                    applicationId);
        if (applicationVo.getScholarHksarGsft() != null && applicationVo.getScholarHksarGsft())
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarHksarGsft",
            ...
            deleteDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarHksarGsft",
                    "HKSAR Government Scholarship Fund Targeted Scholarship", applicationId);
        if (applicationVo.getScholarHksarGsftBrs() != null && applicationVo.getScholarHksarGsftBrs())
            addDocumentRequirement(Constants.DOC_TYPE_OTHERS, "ScholarHksarGsftBrs",
        ...
        try {
            if (StringUtils.isNotBlank(firstChoiceProgId) && !"0".equals(firstChoiceProgId)) {
                ...
                if (StringUtils.isBlank(firstProgChoice.getId())) {
                    ...
                    firstProgChoice.setUniCompReq(false);
                }
            ...
            for (ProgrammeChoiceVo othersProgChoice : othersProgChoices) {
                if (StringUtils.isNotBlank(othersProgChoice.getAdmFormProgId())
                    ...
                    if (StringUtils.isBlank(othersProgChoice.getId())) {
                        ...
                        othersProgChoice.setUniCompReq(false);
                    }
            ...
            if (firstProgChoice != null) {
                ...
                AuditLogUtil.saveAuditLog(accountVo.getId(), Constants.SAVED_CHOICE_OF_PROGRAMME);
            }
    ...
    public void withDraw(String id) {
        ...
        if (StringUtils.isNotBlank(id)) {
            ...
            if (appEndDate.getTime() > new Date().getTime()) {
                ...
                FormProgVo formProgVo = formProgService.get(programmeChoice.getAdmFormProgId());
                deleteDocumentRequirement(Constants.DOC_TYPE_PIS, formProgVo.getProgrammeCode(),
                        "Personal Statment for HKU-UCL Dual Degree", applicationId);
            } else {
                if (!programmeChoice.getFirstChoice()) {
                    for (int i = 0; i < othersProgChoices.size(); i++) {
