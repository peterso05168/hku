...
CREATE PROCEDURE calculate_predicted_actual_score_IBD_45()
BEGIN
	...
	END IF;
		...
		BEGIN
			...
			DECLARE qualification_rslt_list CURSOR FOR (
				...
				AND ((a.ib_achieved_rslt_grade_cd IS NOT NULL AND a.ib_achieved_rslt_grade_cd != '' AND a.ib_achieved_rslt_grade_cd != 'N - No grade')
					OR (a.ib_predicted_rslt_grade_cd IS NOT NULL AND a.ib_predicted_rslt_grade_cd != '' AND a.ib_predicted_rslt_grade_cd != 'N - No grade'))
			...
			AND calculate_type = 'Predicted and Actual' AND out_of = '45';
			...
			SELECT IF((a.ib_achieved_rslt_grade_cd IS NOT NULL AND a.ib_achieved_rslt_grade_cd != '' AND a.ib_achieved_rslt_grade_cd != 'N - No grade') OR (a.ib_predicted_rslt_grade_cd IS NOT NULL AND a.ib_predicted_rslt_grade_cd != '' AND a.ib_predicted_rslt_grade_cd != 'N - No grade'),
				IF((a.ib_achieved_rslt_grade_cd IS NOT NULL AND a.ib_achieved_rslt_grade_cd != '' AND a.ib_achieved_rslt_grade_cd != 'N - No grade') AND (a.ib_predicted_rslt_grade_cd IS NOT NULL AND a.ib_predicted_rslt_grade_cd != '' AND a.ib_predicted_rslt_grade_cd != 'N - No grade'),
					IF(a.ib_predicted_rslt_grade_cd > a.ib_achieved_rslt_grade_cd,a.ib_predicted_rslt_grade_cd,a.ib_achieved_rslt_grade_cd),
					IF(a.ib_achieved_rslt_grade_cd IS NOT NULL AND a.ib_achieved_rslt_grade_cd != '' AND a.ib_achieved_rslt_grade_cd != 'N - No grade',a.ib_achieved_rslt_grade_cd,a.ib_predicted_rslt_grade_cd)
				),
				0
			) INTO ee_tok 
			FROM app_qualification a WHERE a.app_qualification_id = qualification_id;
			SELECT total_rslt + ee_tok INTO total_rslt;
			...
			INSERT INTO app_ibd_best_exam (app_ibd_best_exam_id,application_id,app_qualification_id,total_rslt,calculate_type,out_of) 
						VALUES (@ibd_best_exam_id,applicationid,qualification_id,total_rslt,'Predicted and Actual','45');
...
hkuSql.execute('DROP PROCEDURE IF EXISTS calculate_predicted_actual_score_IBD_45')
hkuSql.execute(sql)