...
CREATE PROCEDURE calculate_actual_score_GCE()
BEGIN
	...
	END IF;
		...
		BEGIN
			...
			DECLARE qualification_rslt_list CURSOR FOR (
				SELECT c.app_qualification_rslt_id,d.exam_subject_desc,b.exam_type_month,b.exam_type_year,c.achieved_grade_cd 
			...
			DELETE FROM app_best_exam_subj_rslt WHERE app_best_exam_subj_id = (
				...
				AND exam_type_id = examTypeId AND calculate_type = 'Actual'
			);
			DELETE FROM app_best_exam_subj WHERE application_id = applicationid AND exam_type_id = examTypeId AND calculate_type = 'Actual';

			OPEN qualification_rslt_list;
				qualification_rslt_items : LOOP
					FETCH qualification_rslt_list INTO qualification_rslt_id,subjectDesc,examTypeMonth,examTypeYear,examGradeCd;
			...
			IF number = 1 THEN
				...
				INSERT INTO app_best_exam_subj (app_best_exam_subj_id,exam_type_id,application_id,calculate_type) VALUES (best_exam_subj_id,examTypeId,applicationid,'Actual');
			END IF;
			...
			INSERT INTO app_best_exam_subj_rslt (app_best_exam_subj_rslt_id,app_best_exam_subj_id,app_qualification_rslt_id,seq_no,exam_subject_desc,exam_type_month,exam_type_year,exam_grade_cd,rslt_type) 
						VALUES (@best_exam_subj_rslt_id,best_exam_subj_id,qualification_rslt_id,seq_no,subjectDesc,examTypeMonth,examTypeYear,examGradeCd,'Achieved');
...
hkuSql.execute('DROP PROCEDURE IF EXISTS calculate_score_GCE')
hkuSql.execute('DROP PROCEDURE IF EXISTS calculate_actual_score_GCE')
hkuSql.execute(sql)